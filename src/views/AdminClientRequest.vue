<template>
  <EmployeeLayout>
    <template #title>{{ $t('offer.title') }}</template>
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold">{{ $t('offer.title') }}</h2>
      <div class="flex items-center space-x-4 space-x-reverse">
        <button class="btn-primary mx-3" @click="showVisit = true">
          <i class="pi pi-plus mr-2"></i> {{ $t('offer.add_visit') }}
        </button>
        <button class="btn-primary mx-3" @click="showFollowUp = true">
          <i class="pi pi-plus mr-2"></i> {{ $t('offer.add_follow_up') }}
        </button>
        <button class="btn-primary" @click="showReq = true">
          <i class="pi pi-plus mr-2"></i> {{ $t('offer.add_requirement') }}
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <!-- Visits Table -->
      <div class="card">
        <h3 class="text-lg font-semibold mb-4 text-primary-600">{{ $t('offer.visits') }}</h3>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="visit in visits" :key="visit.id">
                ...
                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                  <span v-if="visit.notes" class="text-green-600">{{ $t('offer.reported') }}</span>
                  <span v-else class="text-red-600">{{ $t('offer.not_reported') }}</span>
                </td>
      ...
      <!-- Follow-ups Table -->
      <div class="card">
        <h3 class="text-lg font-semibold mb-4 text-blue-600">{{ $t('offer.follow_ups') }}</h3>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
              <tr>
      ...
      <!-- Requirements Table -->
      <div class="card">
        <h3 class="text-lg font-semibold mb-4 text-green-600">{{ $t('offer.requirements') }}</h3>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
              <tr>
    ...
    <div v-if="showVisit" class="modal-overlay">
      <div class="modal-content p-4" style="max-width:400px">
        <h3 class="text-lg font-semibold mb-4">{{ $t('offer.add_visit') }}</h3>
        <form @submit.prevent="addVisit()">
          ...
          <div class="flex justify-end space-x-2 space-x-reverse">
            <button type="button" class="btn-secondary" @click="closeVisitModal">{{ $t('buttons.cancel') }}</button>
    ...
    <div v-if="showVisitReport" class="modal-overlay">
      <div class="modal-content p-4 max-w-2xl" style="max-width: 500px;">
        <h3 class="text-lg font-semibold mb-4">
          {{ editingVisitReport ? $t('offer.edit_visit_report') : $t('offer.add_visit_report') }}
        </h3>
        <form @submit.prevent="saveVisitReport()">
          <textarea
            v-model="visitReportForm.report"
            :placeholder="$t('offer.visit_report') + '...'"
    ...
    <div v-if="showFollowUp" class="modal-overlay">
      <div class="modal-content p-4" style="max-width:400px">
        <h3 class="text-lg font-semibold mb-4">{{ $t('offer.add_follow_up') }}</h3>
        <form @submit.prevent="addFollowUp()">
          <select v-model="followUpForm.type" class="input-field mb-2" required>
            <option value="">{{ $t('offer.follow_up_type') }}</option>
            <option value="external">{{ $t('offer.follow_up_type_external') }}</option>
            <option value="internal">{{ $t('offer.follow_up_type_internal') }}</option>
            <option value="phone">{{ $t('offer.follow_up_type_phone') }}</option>
          </select>
          <input
          ...
          <textarea
            ...
            :placeholder= "$t('offer.follow_up') + '...'"
            class="input-field mb-2 h-24"
            required
          ></textarea>
          <div class="flex justify-end space-x-2 space-x-reverse">
            <button type="button" class="btn-secondary" @click="closeFollowUpModal">{{ $t('buttons.cancel') }}</button>
            <button type="submit" class="btn-primary">{{ $t('buttons.save') }}</button>
          </div>
        </form>
      </div>
    ...
    <div v-if="showReq" class="modal-overlay">
      <div class="modal-content p-4" style="max-width:400px">
        <h3 class="text-lg font-semibold mb-4">{{ editingReq ? $t('offer.edit_requirement') : $t('offer.add_requirement') }}</h3>
        <form @submit.prevent="editingReq ? updateReq() : addRequirement()">
          <select v-model="reqForm.service" class="input-field mb-2" required>
            <option value="" disabled>{{ $t('offer.service') }}</option>
            <option v-for="s in services" :key="s.id" :value="s.label">{{ s.label }}</option>
          </select>
          <textarea
            v-model="reqForm.description"
            :placeholder="$t('offer.requirement') + '...'"
            class="input-field mb-2"
            required
          ></textarea>
          <div class="flex justify-end space-x-2 space-x-reverse">
            <button type="button" class="btn-secondary" @click="closeReqModal">{{ $t('buttons.cancel') }}</button>
            <button type="submit" class="btn-primary">{{ $t('buttons.save') }}</button>
          </div>
        </form>
      </div>
    ...
    <div v-if="showFollowUpDetails" class="modal-overlay">
      <div class="modal-content p-4 max-w-lg" style="max-width: 500px;">
        <h3 class="text-lg font-semibold mb-4">{{ $t('offer.follow_up_details') }}</h3>
        <div class="mb-4">
          <strong>{{ $t('offer.follow_up_type') }}:</strong> {{ getFollowUpTypeText(selectedFollowUp?.service_title_ar || "") }}<br />
          <strong>{{ $t('offer.date_time') }}:</strong> {{ selectedFollowUp?.date_time || ""}}<br />
          <strong>{{ $t('offer.details') }}:</strong><br />
          <p class="mt-2 p-3 bg-gray-100 dark:bg-gray-700 rounded">
            {{ selectedFollowUp?.notes }}
          </p>
        </div>
        <div class="flex justify-end">
          <button class="btn-secondary" @click="showFollowUpDetails = false">{{ $t('offer.close') }}</button>
        </div>
      </div>
    </div>
  </EmployeeLayout>
...
import EmployeeLayout from "@/components/AdminLayout.vue";
...
const services = ref([
  {
    id : 1 ,
    label : t('offer.design_website')
  },
  {
    id : 2 ,
    label : t('offer.design_e_commerce_website')
  },
  {
    id : 3,
    label : t('offer.design_mobile_app')
  },
  // {
  //   id : 4 ,
  //   label : 'نظام erp'
  ...
  {
    id : 5 ,
    label : t('offer.system_erp')
  },
  {
    id : 6 ,
    label : t('offer.hosting_service')
  },
  {
    id : 7 ,
    label : t('offer.social_media_management')
  },
  {
    id : 8 ,
    label : t('offer.photography_video')
  },
  {
    id : 9,
    label : t('offer.design_branding')
  },
  {
    id : 10,
    label : t('offer.design_video')
  },
])
...
function getFollowUpTypeText(type: string) {
  const types = {
    external: t('offer.external'),
    internal: t('offer.internal'),
    phone: t('offer.phone'),
  };
